...
export default Ember.Component.extend({
    didInsertElement() {
        ...
        domEle.children(".bar--aps").find(".bar__bg").css("width", (attacksPerSecond-minAps)/(maxAps-minAps)*100 + "%");
        domEle.children(".bar--burst").find(".bar__bg").css("width", (burstDamage-minBurst)/(maxBurst-minBurst)*100 + "%");
        domEle.children(".bar--dps").find(".bar__bg").css("width", (damagePerSecond-minDps)/(maxDps-minDps)*100 + "%");
    ...
    handleBarsUpdate: Ember.observer('updatedStats', function() {
        this._super(...arguments);
        var heroData = this.get('heroData');
        var attacksPerSecond = heroData.AttacksPerSecond;
        var burstDamage = heroData.BurstDamage;
